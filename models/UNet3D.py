...
class UNet3D(BaseModel):
    def __init__(self, sess, checkpoint_dir, log_dir, training_root, testing_root, training_subjects, testing_subjects,
                 testing_during_training, model_config):
        ...
        self.use_mr_augmentation = model_config['use_mr_augmentation'] == 'True'
        self.use_adversarial_training = model_config['use_adversarial_training'] == 'True'
        
        self.test_nstride = int(model_config['test_nstride'])
        self.voxel_spacing = list(map(int, model_config['voxel_spacing'].split(',')))
        
        # For simplicity, these values are just hard-coded
        self.input_features = 2
        self.nclass = 4
        #self.class_labels = [0, 10, 150, 250]
